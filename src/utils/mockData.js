...
setInterval(() => {
  mockData.regionData.regions.forEach(region => {
    region.value = utils.createRandom(1, 100);
  });

  mockData.verData.servers.forEach(server => {
    server.value = utils.createRandom(1, 100);
  });

  mockData.riskData.risks.forEach(risk => {
    risk.value = utils.createRandom(1, 100);
  });

  mockData.abnormalData.abnormals.forEach(abnormal => {
    abnormal.value = utils.createRandom(300, 500);
  });

  mockData.relationData.relations.forEach(relation => {
    relation.speed = utils.createRandom(1, 200);
    relation.value = [utils.createRandom(0, 300), utils.createRandom(0, 300)];
  });

  mockData.wordCloudData.datas.forEach(data => {
    data.value = utils.createRandom(1, 100);
  });

  mockData.totalData.db = utils.createRandom(10000, 90000);
  mockData.totalData.hb = utils.createRandom(10000, 90000);
  mockData.totalData.hd = utils.createRandom(10000, 90000);
  mockData.totalData.total = utils.createRandom(100000, 9000000);
  mockData.totalData.xb = utils.createRandom(10000, 90000);
  mockData.totalData.xn = utils.createRandom(10000, 90000);
  mockData.totalData.zn = utils.createRandom(10000, 90000);

  mockData.mapData.categoryData[2019].forEach(item => {
    item.value = utils.createRandom(1, 100);
  });

  mockData.mapData.categoryData[2020].forEach(item => {
    item.value = utils.createRandom(1, 100);
  });

  mockData.mapData.categoryData[2021].forEach(item => {
    item.value = utils.createRandom(1, 100);
  });

  mockData.mapData.categoryData[2022].forEach(item => {
    item.value = utils.createRandom(1, 100);
  });

  mockData.mapData.categoryData[2023].forEach(item => {
    item.value = utils.createRandom(1, 100);
  });

  mockData.mapData.topData[2019].forEach(item => {
    item.value = [utils.createRandom(100, 120), utils.createRandom(30, 40), utils.createRandom(50, 60)];
  });

  mockData.mapData.topData[2020].forEach(item => {
    item.value = [utils.createRandom(100, 120), utils.createRandom(30, 40), utils.createRandom(50, 60)];
  });

  mockData.mapData.topData[2021].forEach(item => {
    item.value = [utils.createRandom(100, 120), utils.createRandom(30, 40), utils.createRandom(50, 60)];
  });

  mockData.mapData.topData[2022].forEach(item => {
    item.value = [utils.createRandom(100, 120), utils.createRandom(30, 40), utils.createRandom(50, 60)];
  });

  mockData.mapData.topData[2023].forEach(item => {
    item.value = [utils.createRandom(100, 120), utils.createRandom(30, 40), utils.createRandom(50, 60)];
  });
